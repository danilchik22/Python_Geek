...
index_word = 0
while index_word < len(list_words):
    ...
    for letter in range(len(list_words[index_word])):
        if numbers.count(list_words[index_word][letter]) == 0:  # If there is at least one not figure - not number
            ...
            if letter == 0 and list_words[index_word][letter] == '+' and len(list_words[
                ...
                flag = True
    if flag:
        list_words.insert(index_word, '"')
        list_words.insert(index_word + 2, '"')
        if len(list_words[index_word + 1]) == 1:
            list_words[index_word + 1] = '0' + list_words[index_word + 1]
        index_word += 2
    index_word += 1
